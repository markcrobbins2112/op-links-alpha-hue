...
- **Unique Heading IDs (Serialization to prevent MD024 violations)**:
  ...
  - **Log Entry Heading & Anchor ID Standards**:
  - **Heading Format**: Use the strict dated entry heading syntax: `### 📅 YYYY-MM-DDTHH:MM:SSZ [vX.Y.Z]` (e.g., `### 📅 2026-07-24T05:05:00Z` or `### 📅 2026-07-07T19:38:00Z v1.24.25`). Do not append descriptive titles after a dash on the entry heading line.
  - **Entry Anchor ID**: Formatted as `#a-YYYYMMDDTHHMMSSZ` with a corresponding TOC return link (e.g., `<a id="a-20260724t050500z"></a>[TOC](#toc-20260724t050500z)`).
  - **Child Subheading Anchor IDs**: Subheadings within a log entry (e.g., `#### 🎯 Primary Goals & Requirements`, `#### 🛠️ Completed Changes in this Session`, `#### 🔸 Affected Files`) **MUST** append the parent log entry's timestamp tag to their anchor IDs (e.g., `<a id="a-primarygoalsrequirements-20260724t050500z"></a>[TOC](#toc-primarygoalsrequirements-20260724t050500z)`). This guarantees unique anchor IDs across all log entries in `LOG.md` and `ARCHIVE.md` without collisions.
