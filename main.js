...
module.exports = class LinkSpectrumPlugin extends Plugin {
  async onload() {
    ...
    // 1. FOR LIVE PREVIEW / EDIT MODE: Polling sweep looking for legacy parent link wrappers
  ...
  // Locates parent containers, finds the last child span, reads it, and stamps the attribute token
  processLegacyLinkSpans() {
    // Select all target link parent containers active in the DOM tree
    const parentLinks = document.querySelectorAll('.cm-s-obsidian .cm-hmd-internal-link, .cm-s-obsidian  .cm-link');
    
    if (parentLinks.length === 0) {
      console.debug('[Link Spectrum Scan] Heartbeat check: 0 active parent .cm-hmd-internal-link selectors found in view.');
      return;
    }
    ...
    parentLinks.forEach((parent, index) => {
      // Isolate the very last child span node where the actual link text content lives
      const lastSpan = parent.querySelector('span:last-child');
      
      if (!lastSpan) {
        console.warn(`[Link Spectrum Warning] Item [${index}] missing a valid inner span block layout inside parent:`, parent);
        return;
      }
      ...
      if (match) {
        // FIX: Extract index [0] from the match results array before casting lowercase conversion
        ...
        if (lastSpan.getAttribute('data-alpha-character') !== firstLetter) {
          ...
          console.log(`%c[Link Spectrum Mutation]%c Stamped attribute [data-alpha-character="${firstLetter}"] onto span node successfully.`, 'color: #2ed573; font-weight: bold;', 'color: default;');
        }
      } else {
  ...
  injectStyles() {
    ...
    let cssRules = `
      /* Clear standard text decorations rules globally across system paths */
    ...
    // Pure CSS @keyframes color animation loops for hovered link states
    cssRules += `
